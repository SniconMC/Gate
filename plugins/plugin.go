...
func startAPIServer(logger logr.Logger, p *proxy.Proxy) {
	...
	http.HandleFunc("/playercount", authMiddleware(func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodGet {
			totalPlayers, err := getTotalPlayerCount(p) // Pass logger here
			if err != nil {
				http.Error(w, err.Error(), http.StatusInternalServerError)
				return
			}
			w.Header().Set("Content-Type", "application/json")
			response := map[string]int{"totalPlayerCount": totalPlayers}
...
func getTotalPlayerCount(p *proxy.Proxy) (int, error) {
	return p.PlayerCount(), nil
}
